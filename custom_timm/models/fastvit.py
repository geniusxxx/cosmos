...
class FastVit(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 获取特征图作为tokens
        tokens = self.forward_features(x)  # [B, C, H, W]
        
        # 获取全局特征（直接使用forward_head）
        global_features = self.forward_head(tokens)  # [B, 512]
        
        # 扁平化并映射tokens
        tokens_flat = tokens.flatten(2).transpose(1, 2)  # [B, H*W, C]
        
        return tokens_flat, global_features
