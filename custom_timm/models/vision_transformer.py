...
default_cfgs = {
    ...
    #tag 
    'vit_so_tau2_patch16_cosmos_224': _cfg(
        hf_hub_id='',
        mean=OPENAI_CLIP_MEAN, std=OPENAI_CLIP_STD, crop_pct=1.0, num_classes=512),
}
...
@register_model
def vit_so_tau2_patch16_cosmos_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(patch_size=16, embed_dim=720, depth=14, num_heads=12, mlp_ratio=1.9806)
    model = _create_vision_transformer('vit_so_tau2_patch16_cosmos_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
